  ...
  case class pulsarUtil(serviceUrl:String) {
   var pulsarClient:org.apache.pulsar.client.api.PulsarClient=null
    ...
    def getPulsarProducer[T](producerName: String,topic:String
      ...
      schema match {
        case Some(schema) =>
          this.pulsarClient.newProducer(schema).topic(topic).producerName(producerName).create
        case None =>
          this.pulsarClient.newProducer.topic(topic).producerName(producerName).create
      }
